...
use crate::report::cbor_to_json;
use c2pa_cbor::decode;
use c2pa_core::jumbf::ParsedStore;
...
pub fn crjson_from_asset(mime: &str, data: &[u8]) -> Option<Json> {
    let format = c2pa_formats::AssetFormat::from_mime(mime)?;
    let store_bytes = c2pa_formats::extract_manifest(format, data).ok()??;
    let store = c2pa_core::jumbf::parse_manifest_store(&store_bytes).ok()?;
...
mod tests {
    ...
    fn store_bytes() -> Vec<u8> {
        ...
        let claim_cbor = c2pa_cbor::encode(&claim, Profile::LegacyPipelineBDefinite).unwrap();
        ...
        let actions_cbor = c2pa_cbor::encode(&actions, Profile::LegacyPipelineBDefinite).unwrap();
