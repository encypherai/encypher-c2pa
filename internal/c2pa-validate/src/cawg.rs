...
fn verify_identity_assertion(
    ...
    let roles_valid = signer_payload
        .get("role")
...
fn identity_timestamp(signature: &[u8], tsa_trust: Option<&TrustList>) -> Option<OffsetDateTime> {
    ...
    let result = c2pa_trust::verify_timestamp_token(token, &payload, trust);
...
fn identity_revocation_status(
    ...
    for (index, subject) in chain.iter().enumerate() {
        ...
        for staple in &staples {
            let Some(evaluation) =
                c2pa_trust::evaluate_ocsp_verified(staple, &issuer, Some(subject), at)
            else {
                continue;
            };
