...
impl VerifyCache {
    ...
    /// [`crate::verify`] and stores the result. The TTL is the cache default.
    ///
    ...
    /// [`crate::VerifyOutput`] is not reconstructed — the caller gets the stored
    /// state + report JSON, which is what a verify response serializes anyway.
    pub fn verify_cached(
        &mut self,
        input: &crate::VerifyInput,
    ) -> Result<(CachedResult, bool), crate::ValidateError> {
...
fn trust_fingerprint(input: &crate::VerifyInput) -> Vec<u8> {
    let mut h = Sha256::new();
...
mod tests {
    ...
    fn verify_cached_miss_then_hit() {
        ...
        let input = crate::VerifyInput {
            ...
            profile: crate::EngineProfile::GENEROUS,
        };
