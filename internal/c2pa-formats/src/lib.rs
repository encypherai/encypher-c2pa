...
pub fn supports_hash_mode(mime: &str) -> bool {
    let canonical = c2pa_core::spec::canonicalize_mime(mime);
...
impl AssetFormat {
    ...
    pub fn from_mime(mime: &str) -> Option<Self> {
        ...
        let canon = c2pa_core::spec::canonicalize_mime(mime);
    ...
    /// certified path.
    pub fn from_mime_for_profile(mime: &str, profile: c2pa_core::EngineProfile) -> Option<Self> {
...
pub use bmff::bmff_exclusion_ranges;
...
pub use bmff::{bmff_fragment_leaf_hash, bmff_merkle_boxes, BmffMerkleBox};
...
mod tests {
    ...
    pub(crate) fn dummy_manifest_store() -> Vec<u8> {
        use c2pa_core::jumbf::{assertion_box, build_manifest, build_manifest_store};
    ...
    fn v2_4_registry_routes_every_mime() {
        use c2pa_core::{spec::mimes_for_version, SpecVersion};
    ...
    fn raw_camera_is_read_only() {
        use c2pa_core::{EngineProfile, OperatingMode, SpecVersion};
