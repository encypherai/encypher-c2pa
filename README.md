...
It does not sign media. The published API contains no signing keys, no COSE signing paths, and nothing that constructs a C2PA manifest or writes one into an asset. That boundary is enforced mechanically: `scripts/check-public-surface.mjs` takes the public API from rustdoc's own output, so re-exports, macro-generated items and impl methods are resolved by the compiler rather than inferred, and diffs it against a reviewed inventory (`public-surface.txt`). CI runs it on every pull request and before every release. For managed signing, policy, durable receipts, or hosted trust decisions, use the [Encypher API](https://api.encypher.com/docs).
