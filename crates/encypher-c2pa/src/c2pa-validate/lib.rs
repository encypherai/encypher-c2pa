...
use crate::c2pa_crypto::{extract_tsa_token, extract_x5chain, timestamp_input, verify_claim};
use crate::c2pa_formats::{text_standard, AssetFormat};
use crate::c2pa_trust::{validate_chain, TrustList};
use serde::Serialize;
...
/// manifests, the set of labels the store contains, and the per-manifest
/// hashed-URI digests. All three are derived from the same parsed store, so
/// they travel together rather than as three positional parameters.
#[derive(Clone, Copy)]
struct StoreContext<'a> {
    manifests: &'a [ParsedManifest<'a>],
    labels: &'a std::collections::HashSet<String>,
...
fn verify_manifest<'a>(
    ...
    verify_ingredient_references(manifest, store.labels, &sig_url, &mut results);
    ...
    let trusted_timestamp = timestamp_token.as_ref().and_then(|token| {
        let verification = match (input.tsa_trust, timestamp_input(cose)) {
            (Some(trust), Ok(timestamp_payload)) => {
                crate::c2pa_trust::verify_timestamp_token(token, &timestamp_payload, trust)
            }
            (None, _) => {
    ...
    if sig_usable {
        ...
        if format == AssetFormat::C2paStore {
            verify_compound_content(manifest, &label, store.manifest_hashes, &mut results);
        }
        ...
        if results.failure.is_empty() {
            cawg::verify_identity_assertions(&mut cawg::IdentityContext {
                manifest,
                manifests: store.manifests,
                manifest_hashes: store.manifest_hashes,
                claim: &claim,
                generation,
                validation_time: at,
    ...
    let staples = crate::c2pa_crypto::extract_ocsp_staples(cose);
    ...
    let mut leaf_status: Option<crate::c2pa_trust::OcspStatus> = None;
    for subject in &chain {
        let Some(issuer_der) = crate::c2pa_trust::resolve_issuer(subject, &issuer_candidates)
        else {
            continue;
        };
        let is_leaf = leaf == Some(subject.as_slice());
        for staple in &staples {
            let Some(eval) = crate::c2pa_trust::evaluate_ocsp_verified(
                staple,
                &issuer_der,
                Some(subject),
                ocsp_at,
            ...
            if eval.status == crate::c2pa_trust::OcspStatus::Revoked {
                ocsp_revoked = true;
                results.push_failure(
    ...
    match leaf_status {
        Some(crate::c2pa_trust::OcspStatus::Good) => results.push_success(
            SIGNING_CREDENTIAL_OCSP_NOT_REVOKED,
            sig_url.clone(),
            "stapled OCSP response: certificate not revoked".into(),
        ),
        Some(crate::c2pa_trust::OcspStatus::Revoked) => { /* failure already recorded */ }
...
fn verify_data_hash(
    ...
    let actual: Option<Vec<u8>> = if is_bmff {
        ...
        crate::c2pa_formats::bmff_hash(data, alg, &xpaths).ok()
    } else {
...
fn verify_bmff_merkle_init(
    ...
    for entry in entries {
        ...
        let actual = match &cached {
            ...
            _ => {
                let h = crate::c2pa_formats::bmff_hash(data, alg, &xpaths).ok();
...
fn verify_bmff_monolithic_entry(
    ...
    let Ok(mdats) = crate::c2pa_formats::bmff_mdat_payloads(data) else {
    ...
    let Ok(aux_boxes) = crate::c2pa_formats::bmff_merkle_boxes(data) else {
        return malformed(results, "asset not parseable as BMFF".into());
    };
...
fn verify_bmff_fragments(
    ...
    for (i, fragment) in fragments.iter().enumerate() {
        ...
        let Ok(leaf) = crate::c2pa_formats::bmff_fragment_leaf_hash(fragment, alg, xpaths) else {
            results.push_failure(
                ASSERTION_BMFF_HASH_MALFORMED,
                url.to_string(),
...
/// manifest box match) is honored inside [`crate::c2pa_formats::bmff_hash`], which
