...
use crate::c2pa_formats::util::be_u24;
...
pub(crate) fn extract(data: &[u8]) -> Result<Option<Vec<u8>>, FormatError> {
    ...
    if start != 0 {
        if let Some(store) = crate::c2pa_formats::id3::extract(data)? {
...
pub(crate) fn strip(asset: &[u8]) -> Result<Vec<u8>, FormatError> {
    ...
    let id3_has_manifest = start != 0 && crate::c2pa_formats::id3::extract(asset)?.is_some();
...
pub(crate) fn embed(asset: &[u8], manifest_store: &[u8]) -> Result<Vec<u8>, FormatError> {
    ...
    crate::c2pa_formats::id3::embed(&clean, manifest_store)
}
...
pub(crate) fn exclusions(data: &[u8]) -> Result<Vec<DataHashExclusion>, FormatError> {
    let start = flac_start(data)?;
    if start != 0 {
        let id3 = crate::c2pa_formats::id3::exclusions(data)?;
        if !id3.is_empty() {
            return Ok(id3);
        }
    }
...
mod tests {
    ...
    fn id3_prefixed_flac_extracts_via_geob() {
        let store = dummy_manifest_store();
        let id3_wrapped = crate::c2pa_formats::id3::embed(&tiny_flac(), &store).unwrap();
        assert_eq!(
